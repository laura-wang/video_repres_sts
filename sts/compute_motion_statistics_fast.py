...
def compute_motion_pattern_1(mag,ang):
    ...
    if max_sum == 0:
        ...
        max_ang_idx = np.argmax(orientation_center) + 1

        return max_mag_idx, max_ang_idx


def compute_motion_pattern_2(mag,ang):
    total_sum = []
    ...
    max_mag_idx = total_sum.index(max(total_sum))

    if max_mag_idx == 0:  # block 1 28 x 28
        max_mag_block = mag[42:70, 42:70]
        max_ang_block = ang[42:70, 42:70]
    elif max_mag_idx == 1:  # block 2 56 x 56
    ...
    elif max_mag_idx == 2:  # block 3 84 x 84
    ...
    elif max_mag_idx == 3:  # block 4 112 x 112
...
def compute_motion_pattern_3(mag,ang):

    mag_block_all = []
    ang_block_all = []

    block_one = mag[0:56,0:56]
    mag_block_1 = block_one[np.tril_indices(56)]  # default: include the diag
    mag_block_2 = block_one[np.triu_indices(56)]

    ang_block_one = ang[0:56, 0:56]
    ang_block_1 = ang_block_one[np.tril_indices(56)]   # default: include the diag
    ...
    mag_block_3 = block_two[np.triu_indices(56)]  # default: include the diag
    ...
    mag_block_5 = block_three[np.triu_indices(56)]  # default: include the diag
    ...
    mag_block_7 = block_four[np.tril_indices(56)]  # default: include the diag
    mag_block_8 = block_four[np.triu_indices(56)]


    ang_block_four = ang[56:112, 56:112]
    ang_block_7 = ang_block_four[np.tril_indices(56)]  # default: include the diag
...
## compute frame with max magnitude based on motion boundary
def compute_motion_global(dx_all, dy_all):

    sum_mag_all = []
    for i in range(15):
        cur_dx = dx_all[i]
        cur_dy = dy_all[i]
        mag, ang = cv2.cartToPolar(cur_dx, cur_dy, angleInDegrees=True)
        sum_mag_all.append(np.sum(mag))

    sum_mag_all = np.array(sum_mag_all)

    return (np.argmax(sum_mag_all) + 1)  # start from 1
